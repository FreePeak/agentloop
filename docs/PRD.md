...
* M4 memory-state: 4 tiers (working/landmark/retrieved/system), 70% context rule enforced on every Add (P32 landmarks never evicted, P34 compress every 5), SQLite WAL checkpoints (durable across process restart, step-5 cadence, ≤4-step loss window), DELETE /v1/runs/{id} endpoint, and resume from a step-5 checkpoint after a step-7 fault (acceptance: 20-iter run holds 70% rule; dedup restored steps 0-4 not re-fired).*
