...
| Model providers (e.g. TypeSafe Jev) | **onegw** `kind = "systemone"`, `POST /v1/systemone` — same OpenAI wire on both sides, no translation | a provider leg onegw already owns (see [`docs/JEV-INTEGRATION.md`](#jev-provider--integration-guide-for-agentloop)); agentloop selects it by tier combo, never directly |
...
| LLM guardrail screening (Noul/Score battery) | **TypeSafe** via onegw `kind = "systemone"` | agentloop sends the screening `state` (user goal before a step, model reply after a step) with a fixed battery of 4 Noul hazard questions + 1 severity Score; TypeSafe returns per-hazard probabilities; agentloop owns the routing thresholds (strict/permissive) and the action mapping; screening is counted in the per-step cost and does not bypass `BudgetGuard` |
...
| M5 | HITL | ApprovalGate, audit, progressive autonomy counters, approval queue UI | <10% interruptions; sampling + anomaly review exercised; timeout denies | **closed** 2026-09-20 (PR #10: ApprovalGate with fail-closed policy table + audit ledger; runner pause on denied gate; 30-minute timeout denies and escalates with partial; M5 acceptance tests in `internal/loop/m5_test.go` — 4 cases) |
| M6 | Evals & console | EvalRunner in CI, REFINE job, HTMX console (trajectory, spend, evals, kill) | deploys blocked on the full-suite gate; +10 cases/week; knee table published | **closed** 2026-09-20 (PR #10: EvalRunner with 4-category suite and pass = score ≥ 0.8 ∧ latency ≤ cap ∧ cost ≤ cap; 4 tests in `internal/eval/eval_test.go`; live HTTP endpoints `GET /admin/api/v1/evals`, `GET /v1/runs/{id}/events`, console pages `GET /admin/console/{runs,approvals}`, `POST /admin/console/kill`) |
...
| **TypeSafe provider coupling** — onegw's `systemone` provider Kind is **merged into master** (`9faea01`); the open blocker is PR #110 (verdict-driven combo reorder) so agentloop's tier selection can actually route to it per step. TypeSafe API spec still unverified. | if the provider surface changes (auth scheme, error codes, model catalog), the guardrail screen and the cost/usage accounting both break silently | gate M2 on PR #110 merge + a spec-verified config block (see `docs/JEV-INTEGRATION.md` §3); treat `TypeSafeClient` as a swappable dependency with its own health probe |
...
- [`docs/JEV-INTEGRATION.md`](#jev-provider--integration-guide-for-agentloop) — the integration guide for the TypeSafe Jev model provider: where it lives (onegw, as a `KindSystemOne` leg), the onegw branch that implements it (`feat/systemone-provider`, unmerged), the config template to add, and the verification checklist. Written 2026-09-18; the Jev API spec at `docs.typesafe.ai` had not been fetched at time of writing.
...
* Last updated: 2026-09-21 (Approval table matched to the real v1 tool names — `query`/`web_search`/`run_tests` are read-category, `write_file` holds on every call. Before this, all four fell to the fail-closed default, so every gated run paused on step 1 and M5's <10% interruption ceiling was unreachable; FR-7 now states the split.) — §5.1 FR-7, plus the walkthrough in `docs/USAGE.md` and `README.md`.*
